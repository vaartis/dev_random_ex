defmodule DevRandom.Platforms.VK do
  def random_post_or_posts do
    ...
    if post_count > 0 do # There are suggested posts
      ...
      if Map.has_key?(suggested_post, "attachments") and (suggested_post["attachments"] |> Enum.filter(fn att -> att["type"] in ["photo", "doc"] end) |> Enum.count) > 0  do
        {:suggested, suggested_post} # Return the whole post
      else
      ...
      case maybe_album_query do
        {:error, _}-> random_post_or_posts() # The user's account is probably deleted or something, just restart
        {:ok, album_query} ->
